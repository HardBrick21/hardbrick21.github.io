...
| ?            | 问号         | 匹配零次或一次                                               |
| *            | 星号         | 可以匹配任意多次，也可能不匹配                               |
| +            | 加号         | 至少需要匹配一次，至多可以任意多次                           |
| \{min, max\} | 区间量词     | 至少需要min次，至多max次                                     |
| ^            | 脱字符       | 匹配一行的 开头位置                                          |
| $            | 美元符       | 匹配一行的结束位置                                           |
| \|           | Alternation  | 匹配任意分隔的表达式                                         |
| \(……\)       | 括号         | 标记一个子表达式的开始和结束位置。子表达式可以获取供以后使用。「限定多选结构的范围、标注量词作用的元素、为反向引用“捕获”文本」 |
...
### 二、[常用正则整理](http://c.runoob.com/front-end/854)
